...
public class Game
{
  ...
  protected boolean flag,isOnline,wait;
        ...
        if (possible.algCoord().compareTo(destin.toString()) == 0)
          {
			...
			save[saveSize++]=getPosition();
            
            board[origin.getRow()][origin.getFile()] = ' ';
            board[destin.getRow()][destin.getFile()] = piece;
			
			if(isOnline){
				if(toMove == OnlineControl.onlinecontrol.getTurn()){
					setFlag();
					OnlineControl.get().InputSomething(moveText);
					ChineseChess.runningApplication().returnSetEnabled(true);
				}
				else{
            ...
            toMove ^= TURNSWITCH;
			ChineseChess.runningApplication().setTurnShow(toMove);
            notifyObservers(moveText);
			
			if(saveSize == 1){
				ChineseChess.runningApplication().manualSetEnabled(true);
  ...
  public void newGame(){
	...
	if(OnlineControl.get().getTurn() == 'r')
		flag = false;
	else
		flag = true;
